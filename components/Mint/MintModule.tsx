...
const MintModule = () => {
  ...
  let { data: totalSupply } = useContractSupply(
    ...
    <div className="flex flex-col w-full align-center justify-center items-center gap-5">
      ...
      {account?.connector && mintAllowed && (
        <>
          ...
          <input
            ...
            className="text-black bg-neutral w-24 text-center text-3xl py-3 rounded-lg  font-bold self-center"
          />
          <MintController
            min={mintMin}
            max={mintMax}
          ...
          <div className="text-center text-lg">
            <div>Recommended to mint many at a time (maximun {mintMax})</div>
            <div>to economize gas fees</div>
          </div>
        </>
      )}
      <div className="flex flex-col w-2/3 md:w-1/2 self-center gap-2">
        {activeChain && activeChain.id != allowedChain.id ? (
          ...
          <BackoffButton
            stepId={stepId}
            minted={minted ? minted.toNumber() : 0}
          />
        )}
      </div>
      {Number(totalSupply) > maxSupply / 2 && (
